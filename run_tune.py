"""
run_tune.py
Runner script that executes the CV grid for all preprocessing variants,
creates summary CSVs via src/tune_cv, and plots mean metric vs K per variant.
Saves plot to reports/figs/knn_cv_metric_vs_k.png
"""
...
def plot_metric_vs_k(agg_df: pd.DataFrame, outpath: str):
    """
    Plot mean metric vs K with error bars (std). agg_df must contain
...
def main():
    ...
    # Suggest candidate K per variant using elbow + stability heuristic:
    candidates = {}
    for variant, group in agg_df.groupby("preprocessing_variant"):
        # Choose K with highest mean_metric; if ties use the one with smallest std_metric

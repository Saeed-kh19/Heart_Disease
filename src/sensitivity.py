"""
src/sensitivity.py
Sensitivity and robustness checks for preprocessing variants.

Implements:
- compare_variants: reads final_metrics.csv and knn_cv_agg.csv and writes a one-paragraph comparison
  between Variant A and Variant B (test F1 and CV stability).
- analyze_scaler_impact: for each variant, computes mean pairwise Euclidean distance on the training set
  before and after scaling (uses the saved preprocessor or scaler when available), compares distances,
  and compiles a short note. Also reports test metric differences between variants.

Outputs:
- reports/results/sensitivity_comparison.txt  (one-paragraph comparison of Variant A vs B)
- reports/results/scaler_impact.txt           (summary of scaler effects and numeric outputs)
- reports/figs/scaler_distance_change.png     (bar plot mean distances before/after by variant)
"""
...
def compare_variants(out_path: str = os.path.join(RESULTS_DIR, "sensitivity_comparison.txt")) -> str:
    """
    Creates a one-paragraph comparison between Variant A and Variant B using:
      - test F1 (from final_metrics.csv)
      - CV mean and std (from knn_cv_agg.csv) aggregated per variant at chosen K
    Saves the paragraph to out_path and returns the paragraph string.
    """
    ensure_dir(os.path.dirname(out_path) or ".")
    final = _load_final_metrics()
    agg = _load_cv_agg()

    # Find rows for VariantA and VariantB in final metrics
...
def analyze_scaler_impact(out_txt: str = os.path.join(RESULTS_DIR, "scaler_impact.txt"),
    ...
    """
    For each variant: attempt to load processed training X (from data/processed/<variant>/..._X_train.csv).
    Compute mean pairwise distance on raw features (if available pre-scaler) and on processed features.
    Because raw pre-scaling arrays are not always stored, attempt to recover pre-scaler data by:
      - loading preprocessor.pkl and inverting scaling where possible (best-effort),
      - otherwise compute distances on processed data and report relative differences between variants.
    Save a textual summary and a bar plot comparing mean distances (processed) between variants.
    Returns a dict of numeric results.
    """
    ...
    for var in PREPROCESSING_VARIANTS:
        ...
        if not os.path.isfile(X_train_path):
            # skip variant if no training split present
            continue

        X_train = pd.read_csv(X_train_path).values

        # mean distance on processed (post-scaling) data
        mean_dist_processed = _compute_mean_pairwise_distance(X_train, sample_frac=0.2, rng_seed=0)

        # Attempt to compute approximate pre-scaled mean distance:
        mean_dist_before = None
        if os.path.isfile(scaler_path):
            try:
                with open(scaler_path, "rb") as f:
                    scaler = pickle.load(f)
                # If scaler is a sklearn scaler with mean_ and scale_, attempt inverse_transform
                try:
    ...
    # Create bar plot comparing after distances (and before if available)
    ...
    # Also return numeric dict for further programmatic use
